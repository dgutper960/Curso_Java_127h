...
import ManejoDeErrores.EnProyectoRepositorioCRUD.excepciones.EscrituraAccesoDatosException;
...
/**
 * LAS POSIBLES EXCEPCIONES, DEBEN SER MANEJADAS EN UN BLOQUE TRY/CATCH
 * --> Se debe respetar el orden de las específicas a las genéricas
 * Primero deben ponerse las Excepciones hijas y luego las padre
 * **/
...
//        ordenAsc2.forEach(System.out::println);
        ...
        }catch (LecturaAccesoDatoException lectDatos){
            lectDatos.getMessage();
            lectDatos.getStackTrace();
        }catch (EscrituraAccesoDatosException escDatos){
            escDatos.getMessage();
            escDatos.getStackTrace();
        }catch (AccesoDatoException accDatos){
            accDatos.getMessage();
            accDatos.getStackTrace();
        }
